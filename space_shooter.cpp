...
void SpaceShooterGame(Engine::Window *window) {
    ...
    while (!window->GetKeystate(Engine::Scancode::SCANCODE_ESCAPE)) {
        ...
        if (!paused) {
            auto group = engine->GetGroupView<Transform2D, Velocity2D, Collider, Scaling>();
            auto b = group.begin();
            auto e = group.end();
            ...
            for (const auto &[transform2D, velocity2D, colliderOriginal, scaling] : engine->GetGroupView<Transform2D, Velocity2D, Collider, Scaling>()) {
                ...
                float remainingTimeX = dt;
                float remainingTimeY = dt;

                if (collider.x + collider.w > windowPos.w) {
                    remainingTimeX = (collider.x - windowPos.w - collider.w) / velocity2D.x;
                ...
                if (collider.x < 0) {
                    remainingTimeX = (-collider.x) / velocity2D.x;
                    velocity2D.x = -velocity2D.x;
                ...
                if (collider.y + collider.h > windowPos.h) {
                    remainingTimeY = (collider.y - windowPos.h - collider.h) / velocity2D.y;
                ...
                if (collider.y < 0) {
                    remainingTimeY = (-collider.y) / velocity2D.y;
                    velocity2D.y = -velocity2D.y;
                    collidedY = true;
